...
// 2 
let output = dogs.filter(it => it.owners.includes('Sarah'));
// console.log("sarahDog :", output[0])

// the above code can also be written like this
const sarahDog = dogs.find(item => item.owners.includes('Sarah'));
// console.log("sarahDog1 :", sarahDog)

// 3
const ownersEatTooMuch = dogs.filter(item => item.curFood > item.recFood).map(item => item.owners).flat()
